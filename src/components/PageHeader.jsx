...
/**
 * Banner sólido para páginas que no son el Inicio (sin video, para no competir con el Hero).
 * ctaAccent: "red" | "blue" -> color del botón de alto contraste (opcional).
 */
export default function PageHeader({ eyebrow, title, subtitle, ctaLabel, ctaTo, ctaAccent = "red" }) {
  const accentBg = ctaAccent === "blue" ? ACCENT.blueVivid : ACCENT.red;
  const accentHover = ctaAccent === "blue" ? ACCENT.blueVividHover : ACCENT.redHover;
  ...
  return (
    <section className="relative bg-[#1B4965] overflow-hidden">
      ...
      >
        ...
        <rect x="0" y="0" width="1440" height="400" fill="url(#pageHeaderGrad)" />
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-10 py-16 md:py-24 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
        ...
        {ctaLabel && ctaTo && (
          <Link
            to={ctaTo}
            className="inline-flex items-center gap-2 rounded-full pl-2 pr-6 py-2 text-white text-sm font-semibold shadow-lg transition-colors w-fit shrink-0"
